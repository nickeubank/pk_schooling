...
eststo: xi: reg fee mauza_zaat_frac  dist2 dist3 if school_private==1, cluster(mauzaid) 
eststo: xi: reg fee mauza_zaat_frac mauza_wealth mauza_gini_expend dist2 dist3 if school_private==1, cluster(mauzaid) 
eststo: xi: reg fee mauza_zaat_frac mauza_wealth mauza_gini_expend dist2 dist3 if school_private==1 [pw=number_students], cluster(mauzaid) 


esttab  using $pk/docs/regressions/fees.tex, b(a2) replace nogaps compress label booktabs noconstant ///
 title(Annual School Fees\label{fees}) ///
	mtitle("Weighted by School" "Weighted by School" "Weighted by Primary Students") ///
	indicate(District Fixed Effects =dist*) wrap ///
