...
count
	...
	* *********
	...
	esttab  using $pk/docs/results/private_share.tex, b(a2) replace nogaps compress label booktabs noconstant ///
	...
	restore
		...
		* *********
		...
		graph export $pk/docs/results/wealth_and_type.pdf, replace
...
preserve
	...
	* Do twice
	eststo:	xi: reg child_`x'_theta school_private  interact_zaat_private ///
		...
		graph save $pk/docs/results/privatepremium_g3_bigsample_`x'.gph, replace
	...
	xi: areg child_`x'_theta school_private mauza_zaat_frac interact_zaat_private ///
		...
		graph save $pk/docs/results/privatepremium_g3_smallsample_`x'.gph, replace

		graph combine ///
			$pk/docs/results/privatepremium_g3_bigsample_`x'.gph $pk/docs/results/privatepremium_g3_smallsample_`x'.gph , ///
		...
		graph export $pk/docs/results/privatepremium_`x'_crosssection.pdf, replace
...
esttab  using $pk/docs/results/privatepremium_crossection.tex, b(a2) replace nogaps compress label booktabs noconstant ///
...
local upper_x=proper("`x'")
		...
		* Do twice with no lag (for control) -- one for interact, one for outreg.
		...
		xi: areg child_`x'_theta L.school_private L.mauza_zaat_frac L.interact_zaat_private ///
			...
			graph save $pk/docs/results/twoyear_nolag_`x'.gph, replace
		...
		* Do twice with lag, big sample. Once for interact, one for outreg.
		...
		xi: areg child_`x'_theta L.school_private L.mauza_zaat_frac L.interact_zaat_private ///
			...
			graph save $pk/docs/results/twoyear_big_`x'.gph, replace
		...
		graph save $pk/docs/results/twoyear_small_`x'.gph, replace



		graph combine ///
			$pk/docs/results/twoyear_nolag_`x'.gph  $pk/docs/results/twoyear_big_`x'.gph ///
			  $pk/docs/results/twoyear_small_`x'.gph, row(1) ///
		...
		graph export $pk/docs/results/twoyear_`x'.pdf, replace
